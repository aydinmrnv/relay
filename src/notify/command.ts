...
import { digestRun } from './format.ts';

export function completionArgs(template: readonly string[], state: RunState): string[] {
  const digest = digestRun(state);
  const values = {
    '{{runId}}': state.runId,
    '{{outcome}}': state.phase.toLowerCase(),
    '{{url}}': state.pullRequest?.url ?? state.issue?.url ?? '',
    // Each token is one argv element's worth of text, never parsed by a shell.
