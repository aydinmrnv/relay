...
function row(issue: IssueSummary): string {
  ...
  return `${issue.ref ?? `#${issue.number}`}  ${issue.title}${labels}  ${age(issue.createdAt)}`.trimEnd();
}
...
export async function resolvePickedIssue(
  ...
  if (issues.length === 1) return issues[0]!.ref ?? String(issues[0]!.number);
  return await prompter.select('Choose an issue', issues.map((issue) => ({ value: issue.ref ?? String(issue.number), label: row(issue) })), 0);
}
...
export async function confirmClosedIssue(issue: Issue, yes: boolean, prompter: PromptSession): Promise<boolean> {
  ...
  const headline = `Issue ${issue.number !== null ? `#${issue.number}` : issue.key ?? issue.id}`;
