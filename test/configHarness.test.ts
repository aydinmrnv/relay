...
import { agentChecks, enforcementChecks, type AgentCheck } from '../src/cli/checks.ts';
...
describe('config harnesses across the CLI surface', () => {
  ...
  it('doctor names the enforcement per config harness: declared flags, or none at all', async () => {
    const registrations = configHarnessRegistrations(
      parseHarnessesConfig({ mytool: MYTOOL, other: MYTOOL_NO_READONLY }, RESERVED),
    );
    const rows = await enforcementChecks(registrations);

    const declared = rows.find((row) => row.label === 'mytool (config) read-only');
    assert.equal(declared?.status, 'ok');
    assert.match(declared?.detail ?? '', /CLI flags from config \(--sandbox read-only\)/);

    // No readOnly flags means no enforcement to name: a warning, with the fix.
    const missing = rows.find((row) => row.label === 'other (config) read-only');
    assert.equal(missing?.status, 'warn');
    assert.match(missing?.detail ?? '', /no readOnly flags declared/);
    assert.match(missing?.hint ?? '', /review roles/);
  });
